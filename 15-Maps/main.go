...
func main() {
	...
	// how to create a map and access its values

	myFamily := map[string]int{
		"Yusra":  23,
		"Dodo":   18,
		"Faxfax": 14,
	}

	fmt.Println("The age of my sister Dodo is", myFamily["Dodo"])
	fmt.Println(myFamily)
	fmt.Printf("the map : %#v \n", myFamily)

	// also can create map using make
	...
	for v, k := range myFamily {
		fmt.Println(v, k)
	}

	for _, k := range myFamily {
		fmt.Println(k)
	}

	for v := range myFamily {
		fmt.Println(v)
	}
	...
	delete(myFamily, "Dodo") // wouldn't have worked if Dodo wasn't a key - default result would be nill
	fmt.Println(myFamily)

	// comma ok idiom ( useful to help us identify whether a key exists or not )
	fmt.Println("---------------------------------------")
	...
	// the one below combined with statement statement idiom - preferred way !
	if v2, ok := random["3"]; !ok { // idiomatic code is to deal with potential errors first
		fmt.Println("Key didn't exist")
	} else {
		fmt.Println("The value prints", v2)
	}
	...
	m := make(map[string]int) // to store the frequencies
	for _, v := range xs {
		m[v]++
	}
